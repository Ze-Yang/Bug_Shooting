...
def all_gather(data, group=None):
    ...
    #     device = 'cuda' if 'cuda' in data.device else 'cpu'
    # elif isinstance(data, dict) and all([torch.is_tensor(x) for x in data.values()]):
    #     device = 'cuda' if 'cuda' in data.values()[0].device else 'cpu'
    # elif isinstance(data, list) and all([torch.is_tensor(x) for x in data]):
    #     device = 'cuda' if 'cuda' in data[0].device else 'cpu'
    # else:
    #     device = None
    if torch.is_tensor(data):
        device = data.device
    elif isinstance(data, dict) and all([torch.is_tensor(x) for x in data.values()]):
        device = data.values()[0].device
    elif isinstance(data, list) and all([torch.is_tensor(x) for x in data]):
